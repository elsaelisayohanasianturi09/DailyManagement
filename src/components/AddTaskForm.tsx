...
import { Plus, Calendar, Clock, Sparkles, Heart } from 'lucide-react';
...
const AddTaskForm: React.FC<AddTaskFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate async operation with delay for better UX
  ...
  if (showSuccessMessage) {
    return <div className="bg-gradient-to-r from-green-100 to-emerald-100 rounded-2xl p-6 shadow-lg border-2 border-green-200 animate-scale-in">
  ...
  if (!isOpen) {
    return <div className="space-y-4">
        {/* Main Add Button - dengan warna yang lebih terlihat */}
        <Button onClick={() => setIsOpen(true)} className="w-full bg-gradient-to-r from-emerald-400 to-cyan-400 hover:from-emerald-500 hover:to-cyan-500 text-white font-bold text-lg py-6 rounded-2xl shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-xl border-0">
  ...
  return <div className="bg-gradient-to-br from-white to-purple-50 rounded-2xl p-6 shadow-xl border-2 border-purple-200 animate-scale-in">
      {isSubmitting && <div className="absolute inset-0 bg-white/80 rounded-2xl flex items-center justify-center z-10">
          <div className="text-center">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 relative">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Apa yang ingin kamu capai hari ini? 💫" autoFocus className="border-2 border-purple-200 focus:border-purple-400 rounded-xl backdrop-blur-sm bg-[pastel-peach-light] bg-violet-400" />
          </div>
            ...
            <div>
              ...
              <Select value={emoji} onValueChange={setEmoji}>
                ...
                <SelectContent className="bg-white/95 backdrop-blur-sm border-purple-200 rounded-xl">
                  {emojiOptions.map(e => <SelectItem key={e} value={e} className="text-lg hover:bg-purple-50">
                      {e}
                    </SelectItem>)}
                </SelectContent>
              </Select>
        ...
        <div className="flex gap-3 pt-4">
          <Button type="submit" disabled={isSubmitting} className="bg-gradient-to-r from-emerald-400 to-cyan-400 hover:from-emerald-500 hover:to-cyan-500 text-white font-bold flex-1 py-3 rounded-xl shadow-lg transform transition-all duration-200 hover:scale-105">
            <Plus className="w-4 h-4 mr-2" />
            Tambah Task 🎉
          </Button>
          <Button type="button" variant="outline" onClick={() => setIsOpen(false)} className="px-6 border-2 border-purple-200 text-purple-600 hover:bg-purple-50 rounded-xl font-medium">
            Batal
          </Button>
        </div>
      </form>
